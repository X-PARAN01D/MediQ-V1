...
function dayIST(d = new Date()): string {
  return d.toLocaleDateString('en-CA', { timeZone: 'Asia/Kolkata' }) // YYYY-MM-DD
}

async function requireRole(uid: string, roles: string[]): Promise<void> {
  const user = await getAuth().getUser(uid)
  const role = (user.customClaims?.role as string) || 'patient'
  if (!roles.includes(role)) {
    throw new HttpsError('permission-denied', `Requires role: ${roles.join('/')}`)
  }
}

async function requireVerifiedDoctor(uid: string): Promise<{ name: string }> {
  const user = await getAuth().getUser(uid)
  const claims = user.customClaims || {}
  if (claims.role !== 'doctor' || claims.verified !== true) {
    throw new HttpsError('permission-denied', 'Verified doctor required')
  }
  const doc = await db.doc(`users/${uid}`).get()
  return { name: (doc.data()?.name as string) || 'Doctor' }
}

/** Server-side notification write, shared by callables below. */
async function writeNotification(input: {
  userId: string; kind: string; refId?: string; channel: 'sms' | 'inapp' | 'email';
...
async function syncClaims(uid: string, data: FirebaseFirestore.DocumentData | undefined): Promise<void> {
  const role = (data?.role as string) || 'patient'
  const verified = role === 'doctor' ? data?.verified === true : true
  await getAuth().setCustomUserClaims(uid, { role, verified })
}

/* ------------------------------------------------------------------ */
/* 1. Atomic token issuance                                            */
/* ------------------------------------------------------------------ */

interface IssueTokenInput { triageId: string }

export const issueToken = onCall<IssueTokenInput>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  const { triageId } = req.data || {}
  if (!triageId) throw new HttpsError('invalid-argument', 'triageId required')

  const triageRef = db.doc(`triages/${triageId}`)
  const triage = (await triageRef.get()).data()
  if (!triage) throw new HttpsError('not-found', 'Triage not found')
  if (triage.patientId !== req.auth.uid) {
    throw new HttpsError('permission-denied', 'Not your triage')
  }
  const band = triage.band as 'NOR' | 'MOD' | 'EMR'
  if (!['NOR', 'MOD', 'EMR'].includes(band)) {
    throw new HttpsError('failed-precondition', 'Triage has no band yet')
  }
  ...
  interface TokenResult {
    id: string; code: string; band: string; triageId: string;
    patientId: string; patientName: string; status: string;
    createdAt: number; idempotent: boolean;
  }
  ...
  const result = await db.runTransaction(async tx => {
    ...
    const n = ((counter.data()?.seq as number) || 0) + 1
    tx.set(counterRef, { seq: n, day, band, updatedAt: FieldValue.serverTimestamp() }, { merge: true })
    ...
    const r: TokenResult = { id: tokenRef.id, ...token, idempotent: false }
    return r
  })
...
interface SaathiInput { text: string; lang: 'en' | 'hi'; history: { from: string; text: string }[] }

export const saathiChat = onCall<SaathiInput>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  ...
  if (!apiKey) throw new HttpsError('failed-precondition', 'Gemini not configured')
...
interface SmsInput { to: string; body: string; refId?: string }

export const sendSms = onCall<SmsInput>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  await requireRole(req.auth.uid, ['admin'])
  const { to, body } = req.data || {}
  if (!to || !body) throw new HttpsError('invalid-argument', 'to and body required')
...
/* ------------------------------------------------------------------ */
/* 4. Notifications — server-side writer                               */
/*                                                                     */
/* Clients cannot create /notifications directly (rules deny it);       */
/* token events, doctor actions and verification flows call this.       */
/* ------------------------------------------------------------------ */

interface NotifyUserInput {
  userId: string; kind: string; refId?: string;
  channel: 'sms' | 'inapp' | 'email'; title: string; body: string;
}

export const notifyUser = onCall<NotifyUserInput>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  ...
  if (!kind || typeof kind !== 'string') throw new HttpsError('invalid-argument', 'kind required')
  if (!['sms', 'inapp', 'email'].includes(channel)) throw new HttpsError('invalid-argument', 'Bad channel')
...
export const claimToken = onCall<{ tokenId: string }>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  ...
  const outcome = await db.runTransaction(async tx => {
    ...
    const t = snap.data()!
    if (t.status !== 'reserved') return { alreadyClaimed: true as boolean, code: t.code as string }
    tx.update(ref, {
      status: 'active', doctorId: req.auth!.uid, doctorName,
      claimedAt: Date.now(),
    })
    return { alreadyClaimed: false as boolean, code: t.code as string, patientId: t.patientId as string }
  })
...
export const releaseToken = onCall<{ tokenId: string }>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  const { name: doctorName } = await requireVerifiedDoctor(req.auth.uid)
  const { tokenId } = req.data || {}
  if (!tokenId) throw new HttpsError('invalid-argument', 'tokenId required')
  ...
  const outcome = await db.runTransaction(async tx => {
    ...
    if (t.doctorId !== req.auth!.uid && claims.role !== 'admin') {
      throw new HttpsError('permission-denied', 'Only the claiming doctor or an admin can release')
    }
    if (t.status !== 'active') return { changed: false as boolean, code: t.code as string }
    tx.update(ref, {
      status: 'reserved', doctorId: FieldValue.delete(), doctorName: FieldValue.delete(),
      claimedAt: FieldValue.delete(),
    })
    return { changed: true as boolean, code: t.code as string }
  })
...
function istDayStart(d = new Date()): number {
  const ist = new Date(d.getTime() + (330 + d.getTimezoneOffset()) * 60000)
  ist.setUTCHours(0, 0, 0, 0)
  return ist.getTime() - 330 * 60000
}

export const queuePosition = onCall<{ tokenId: string }>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  const { tokenId } = req.data || {}
  if (!tokenId) throw new HttpsError('invalid-argument', 'tokenId required')
  ...
  if (!allowed) throw new HttpsError('permission-denied', 'Not your token')
  ...
  const q = await db.collection('tokens')
    ...
    .get()
  const ahead = q.docs.filter(d =>
    d.id !== tokenId &&
    (d.data().createdAt as number) >= dayStart &&
    (d.data().createdAt as number) < (t.createdAt as number)
  ).length
...
export const bookedSlots = onCall<{ doctorId: string }>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  const { doctorId } = req.data || {}
  if (!doctorId) throw new HttpsError('invalid-argument', 'doctorId required')
  const today = dayIST()
...
export const syncUserClaimsOnCreate = onDocumentCreated('users/{uid}', async event => {
  await syncClaims(event.params.uid, event.data?.data())
})

export const syncUserClaimsOnUpdate = onDocumentUpdated('users/{uid}', async event => {
...
export const bootstrapAdmin = onCall(async req => {
  ...
  if (!existing.empty) throw new HttpsError('already-exists', 'An admin already exists')
  await getAuth().setCustomUserClaims(req.auth.uid, { role: 'admin', verified: true })
  await db.doc(`users/${req.auth.uid}`).set(
    { role: 'admin', verified: true, verification: 'approved', verifiedAt: Date.now() },
    { merge: true },
  )
  return { ok: true }
})
...
export const setDoctorVerification = onCall<{ userId: string; decision: 'approved' | 'rejected'; reason?: string }>(async req => {
  if (!req.auth) throw new HttpsError('unauthenticated', 'Sign in required')
  await requireRole(req.auth.uid, ['admin'])
  ...
  if (decision === 'rejected' && !reason?.trim()) {
    throw new HttpsError('invalid-argument', 'Rejection needs a reason')
  }
  ...
  const doc = snap.data()!
  if (doc.role !== 'doctor') throw new HttpsError('failed-precondition', 'Not a doctor account')

  const patch: Record<string, any> = {
    verification: decision,
    verified: decision === 'approved',
    verifiedAt: Date.now(),
    verificationReason: decision === 'rejected' ? reason!.trim() : FieldValue.delete(),
  }
  ...
  const admin = await getAuth().getUser(req.auth.uid)
  await db.collection('audit').add({
